...
function lookupDeps(file, options) {
  ...
  if (files[src]) {
    //if (!files[src]._done) {
    //  throw new Error('found ' + winPath(relativePath) + ' has recursive dependency');
    //}
...
function requireFile(filepath, pkg, relativePath) {
  try {
    ...
    if (relativePath) {
      e.message += ' that required by ' + winPath(relativePath);
    }
    throw e;
  }
}
...
function getDepFile(filepath, pkg, relativePath) {
  ...
  if (!m) {
    message = 'unknown name ' + winPath(filepath);
    if (relativePath) message += ' required by ' + winPath(relativePath);
    throw new Error(message);
  }
  pkg = pkg.dependencies[m[1]];
  if (!pkg) {
    message = m[1] + ' is not in dependencies';
    if (relativePath) message += ' but required by '  + winPath(relativePath);
    throw new Error(message);
  }
...
function getFileDeps(fullpath) {
  ...
  switch(ext) {
    ...
    case '.css':
      return imports(code).map(cssTransform);
    default:
      return [];
  }
  ...
  function cssTransform(item) {
    if (item.path.charAt(0) === '~') {
      return item.path.slice(1);
    } else if (isRelative(item.path)) {
      return item.path;
    } else {
      return './' + item.path;
    }
