...
var Package = Class.create({
  ...
  _parseFiles: function() {
    ...
    function lookupFiles(src, pkg) {
      ...
      var relativePath = relative(process.cwd(), join(file.pkg.dest, file.path));
      ...
      function getFileInfo(filepath, pkg) {
        ...
        if (!m) {
          throw new Error('unknown name ' + filepath + ' in ' + relativePath);
        }
        pkg = pkg.dependencies[m[1]];
        if (!pkg) {
          throw new Error(m[1] + ' not found but required in ' + relativePath);
        }
