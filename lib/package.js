...
var Package = Class.create({
  ...
  _parseFiles: function() {
    ...
    function lookupFiles(src, pkg) {
      ...
      // file dependencies
      getFileDeps(file.fullpath, file.extension, options)
      ...
      .map(function(name) {
        ...
        if (options.ignore.indexOf(name) !== -1) {
          debug('ignore id(%s) of basepath(%s) of package(%s)', name, src, pkg.id);
          return file.addDeps(name, File.ignore(name));
        }

        // packages of dependency
        if (!isRelative(name)) {
          debug('transport package id(%s) of basepath(%s) of package(%s)', name, src, pkg.id);
          var finfo = util.getFileInfo(name, pkg);
          depFile = File.require(finfo.path, finfo.pkg);
          if (!finfo.pkg.files[depFile.path]) {
        ...
        // relative
        debug('transport relative id(%s) of basepath(%s) of package(%s)', name, src, pkg.id);
        var path = resolvePath(name, src);
        depFile = File.require(path, pkg);
